...
// User alerts table with priority-based evaluation
export const userAlerts = pgTable("user_alerts", {
  ...
  priority: text("priority", { enum: ["HIGH", "MEDIUM", "LOW"] }).notNull().default("MEDIUM"), // HIGH=10s, MEDIUM=1min, LOW=5min
  logicalOperator: text("logical_operator", { enum: ["AND", "OR"] }).default("AND"), // For grouping multiple conditions
  deliveryChannels: text("delivery_channels").array().notNull().default(["email"]), // email, webhook, push, in_app
...
// User broker credentials table for individual user connections
export const userBrokerCredentials = pgTable("user_broker_credentials", {
  ...
  pin: text("pin").notNull(), // Encrypted
  totpKey: text("totp_key").notNull(), // Encrypted
  isActive: boolean("is_active").notNull().default(true),
  ...
  unique("user_broker_unique").on(table.userId, table.brokerType), // One credential per broker per user
]);

// Alert execution log table for tracking alert evaluations
export const alertExecutionLog = pgTable("alert_execution_log", {
  id: serial("id").primaryKey(),
  ...
  metadata: jsonb("metadata"), // Additional context for the alert execution
});
...
// Broker configurations table for encrypted credential storage
export const brokerConfigs = pgTable("broker_configs", {
  id: serial("id").primaryKey(),
  brokerName: text("broker_name", { enum: ["angel-one", "dhan"] }).notNull().unique(),
  clientId: text("client_id").notNull(),
  apiKey: text("api_key").notNull(),
  apiSecret: text("api_secret").notNull(), // AES encrypted
  pin: text("pin").notNull(), // AES encrypted
  totpKey: text("totp_key"), // AES encrypted, optional
  isActive: boolean("is_active").notNull().default(true),
...
// =======================================
// PHASE 6: AI STRATEGY ASSISTANT TABLES
// =======================================
...
export const userPreferences = pgTable("user_preferences", {
  ...
  preferredStrategies: jsonb("preferred_strategies").$type<string[]>().notNull().default([]),
  ...
  preferredInstruments: jsonb("preferred_instruments").$type<string[]>().notNull().default([]),
...
export const userSessions = pgTable("user_sessions", {
  ...
  sessionData: jsonb("session_data").$type<Record<string, any>>().notNull().default({}),
...
export const featureUsage = pgTable("feature_usage", {
  ...
  actionType: varchar("action_type", { length: 50 }).notNull(), // VIEW, CLICK, EXECUTE, etc.
...
// AI strategy recommendations storage
export const aiStrategyRecommendations = pgTable("ai_strategy_recommendations", {
  ...
  strikes: jsonb("strikes").$type<number[]>().notNull().default([]),
  ...
  riskManagement: jsonb("risk_management").$type<string[]>().notNull().default([]),
  exitCriteria: jsonb("exit_criteria").$type<string[]>().notNull().default([]),
  marketContext: jsonb("market_context").$type<Record<string, any>>().notNull().default({}),
  ...
  performance: jsonb("performance").$type<Record<string, any>>().notNull().default({}),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const userAnalytics = pgTable("user_analytics", {
  ...
  sessionDuration: integer("session_duration").notNull().default(0), // in minutes
  featuresUsed: jsonb("features_used").$type<string[]>().notNull().default([]),
  ...
  mostViewedInstruments: jsonb("most_viewed_instruments").$type<string[]>().notNull().default([]),
...
export const aiStrategyRecommendationsRelations = relations(aiStrategyRecommendations, ({ one, many }) => ({
  user: one(users, {
...
export const aiRecommendationFeedbackRelations = relations(aiRecommendationFeedback, ({ one }) => ({
  user: one(users, {
...
// Type exports for Phase 6 tables
export type UserPreferences = typeof userPreferences.$inferSelect;
export type InsertUserPreferences = typeof userPreferences.$inferInsert;
export type UserSessions = typeof userSessions.$inferSelect;
export type InsertUserSessions = typeof userSessions.$inferInsert;
export type FeatureUsage = typeof featureUsage.$inferSelect;
export type InsertFeatureUsage = typeof featureUsage.$inferInsert;
export type AIStrategyRecommendations = typeof aiStrategyRecommendations.$inferSelect;
export type InsertAIStrategyRecommendations = typeof aiStrategyRecommendations.$inferInsert;
...
export const insertAIStrategyRecommendationsSchema = createInsertSchema(aiStrategyRecommendations).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertAIRecommendationFeedbackSchema = createInsertSchema(aiRecommendationFeedback).omit({
  id: true,
  createdAt: true,
});
