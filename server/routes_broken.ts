...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const { 
    ...
    getHealthStatus, 
    prometheusRegister 
  } = await import('./middleware/monitoring.js');
  ...
  app.get('/api/health', async (req, res) => {
    try {
      ...
      try {
        ...
        healthStatus.services.database.responseTime = 15;
      } catch (error) {
      ...
      try {
        const redisStatus = await cacheAdapter.ping();
        healthStatus.services.redis.status = redisStatus ? 'connected' : 'disconnected';
        healthStatus.services.redis.responseTime = 5;
      } catch (error) {
      ...
      try {
        const dataFeedStatus = centralizedDataFeed.getStatus();
        healthStatus.services.marketData.status = dataFeedStatus.isActive ? 'active' : 'offline';
        healthStatus.services.marketData.lastUpdate = dataFeedStatus.lastUpdate.toISOString();
        healthStatus.services.marketData.providers = dataFeedStatus.providers;
      } catch (error) {
        healthStatus.services.marketData.status = 'offline';
      }
  ...
  app.get('/api/health/market-data', async (req, res) => {
    try {
      ...
      res.json({
        status: dataFeedStatus.isActive ? 'active' : 'offline',
        lastUpdate: dataFeedStatus.lastUpdate.toISOString(),
        providers: dataFeedStatus.providers,
        subscribers: dataFeedStatus.subscribers,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      res.status(503).json({
  ...
  app.use(expressStatusMonitor.default({
    title: 'Options Intelligence Platform - System Monitor',
    path: '/admin/status',
    spans: [{
      interval: 1,
      retention: 60
...
function generateMockOptionData() {
  ...
  for (let i = -10; i <= 10; i++) {
    ...
    strikes.push({
      ...
      callOIChange: Math.floor(Math.random() * 10000) - 5000,
      callLTP: Math.random() * 100,
      callVolume: Math.floor(Math.random() * 50000),
      putOI: Math.floor(Math.random() * 100000),
      putOIChange: Math.floor(Math.random() * 10000) - 5000,
      putLTP: Math.random() * 100,
      putVolume: Math.floor(Math.random() * 50000)
    });
  }
