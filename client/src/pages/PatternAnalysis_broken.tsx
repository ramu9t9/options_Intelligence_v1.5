...
interface Pattern {
  id: string;
  type: 'CALL_BUILDUP' | 'PUT_BUILDUP' | 'CALL_UNWINDING' | 'PUT_UNWINDING' | 'GAMMA_SQUEEZE' | 'MAX_PAIN' | 'UNUSUAL_ACTIVITY' | 'SUPPORT_RESISTANCE';
  instrument: string;
  strike?: number;
...
export function PatternAnalysis() {
  ...
  const generatePatternData = () => {
    ...
    const patternData: Pattern[] = [
      ...
      },
      {
        ...
        severity: 'HIGH',
        confidence: 88,
        description: 'Unusual options activity detected in OTM calls',
        signals: [
          'Volume: 10x daily average',
          'Large block trades identified',
          'Institutional flow detected',
          'Cross-market confirmation'
        ],
        timeDetected: new Date(Date.now() - 60 * 60 * 1000),
        recommendation: 'Smart money positioning detected. Monitor for directional confirmation.'
      },
      {
        id: '5',
        type: 'PUT_UNWINDING',
  ...
  const getPatternSentiment = (type: string) => {
    const bullishPatterns = ['CALL_BUILDUP', 'PUT_UNWINDING'];
  ...
  const getPatternIcon = (type: string) => {
    switch (type) {
      case 'CALL_BUILDUP': return <TrendingUp className="w-5 h-5" />;
      case 'PUT_BUILDUP': return <TrendingDown className="w-5 h-5" />;
      case 'GAMMA_SQUEEZE': return <AlertTriangle className="w-5 h-5" />;
      case 'MAX_PAIN': return <Target className="w-5 h-5" />;
      case 'UNUSUAL_ACTIVITY': return <Activity className="w-5 h-5" />;
